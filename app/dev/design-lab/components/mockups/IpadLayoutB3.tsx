...
const MOCK_STEPS = [
  { id: '1', title: '蒸らし（味：40%）', description: '粉全体にまんべんなく注いで湿らせます', startTimeSec: 0, targetTotalWater: 30, note: '蒸らしのお湯を入れたタイミングでタイマーを開始してください。' },
...
/**
 * B-3: サイドバー詳細型
 * 左サイドバーを幅広くしてステップ説明の一部も表示。
 * 右カラムはタイマー（中サイズ）+ ステップカード。
 */
export default function IpadLayoutB3() {
  ...
  return (
    <div className="w-full max-w-[900px] mx-auto">
      <div className="mb-2 flex items-center gap-2">
        <span className="px-2 py-0.5 rounded-full bg-spot text-white text-xs font-bold">B-3</span>
        <span className="text-xs text-ink-muted">サイドバー詳細型・各ステップの説明も表示</span>
      </div>
      ...
      <div className="h-[580px] rounded-2xl border-2 border-edge overflow-hidden flex flex-col bg-ground shadow-lg">
        ...
        <div className="flex-none flex items-center justify-between px-5 pt-4 pb-2 bg-ground">
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs text-ink-muted font-medium">{MOCK_RECIPE_NAME}</span>
        ...
        <div className="flex-1 flex min-h-0">
          ...
          {/* Left: Wide step list with descriptions */}
          <div className="w-[300px] flex-none border-r border-edge bg-surface/30 overflow-y-auto py-2 px-3">
            <div className="space-y-1">
              {MOCK_STEPS.map((step, index) => {
                ...
                return (
                  <div
                    key={step.id}
                    className={`flex items-start gap-2.5 px-3 py-3 rounded-xl transition-colors ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        ...
                        {step.targetTotalWater && (
                          <span className={`text-[11px] font-bold tabular-nums ml-2 flex-none ${
                            isCurrent ? 'text-spot' : isDone ? 'text-ink-muted/50' : 'text-ink-muted'
                          }`}>
                            {step.targetTotalWater}g
                          </span>
                        )}
                      </div>
                      <p className={`text-[10px] tabular-nums mb-1 ${
                        isCurrent ? 'text-spot/70' : isDone ? 'text-ink-muted/50' : 'text-ink-muted'
                      }`}>
                        {formatTime(step.startTimeSec)}〜
                      </p>
                      {/* Brief description — shown for current and upcoming */}
                      {!isDone && (
                        <p className={`text-[10px] leading-snug ${
                          isCurrent ? 'text-ink-sub' : 'text-ink-muted/70'
                        }`}>
                          {step.description}
                        </p>
                      )}
